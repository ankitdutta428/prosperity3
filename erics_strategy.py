...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if "RAINFOREST_RESIN" in state.order_depths:
            amethyst_position = state.position["RAINFOREST_RESIN"] if "RAINFOREST_RESIN" in state.position else 0
            amethyst_orders = self.amethyst_orders(state.order_depths["RAINFOREST_RESIN"], amethyst_fair_value, amethyst_width, amethyst_position, amethyst_position_limit)
            result["RAINFOREST_RESIN"] = amethyst_orders

        if "KELP" in state.order_depths:
            starfruit_position = state.position["KELP"] if "KELP" in state.position else 0
            starfruit_orders = self.starfruit_orders(state.order_depths["KELP"], starfruit_timemspan, starfruit_make_width, starfruit_take_width, starfruit_position, starfruit_position_limit)
            result["KELP"] = starfruit_orders
